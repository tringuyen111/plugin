...
Use this reference when the fixed technical decision materially changes CI/CD, build automation,
artifact publication/promotion, pipeline triggers, runner/execution identity, environment gates,
or delivery evidence production.
...
`codebase-design` owns the technical design decision. `/implement` owns approved repository/config
implementation. When a current Deployment Plan exists, consume its automation/capability/evidence
requirements as authoritative deployment input; do not weaken them to fit an existing pipeline.
See [Deployment Plan](../deploy-release/DEPLOYMENT-PLAN.md).

This reference does **not** own:

- Product behavior, QA verdicts, UAT acceptance, or release readiness;
- `READY_FOR_RELEASE`, which remains owned by `/release-gate`;
- the normal deployment transaction or deployment/exposure state, owned by `/deploy-release`;
...
release owner grants READY_FOR_RELEASE
...
- provider-native deployment stage vs explicit handoff to Deployment Engineering.
...
- deployment/release owner handoffs;
