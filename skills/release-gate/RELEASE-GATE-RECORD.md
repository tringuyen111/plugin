...
- UAT-consumed QA report revision / digest:
- UAT / Release QA report identity match:
...
- QA report / Visual QA / other required evidence:
...
- Operator / provider automation handoff:
...
- Release Gate record revision handed to deployment:
- Release Gate record digest / immutable identity handed to deployment:
- Deployment state owner / reference:
...
An exact `ADMITTED_CURRENT` UAT record is required whenever UAT acceptance is required for the release assessment. Logical UAT IDs, stale/superseded decisions, QA-identity mismatch, or expired/non-applicable UAT conditions must remain visible and cannot be converted into `CONDITIONALLY_READY`. Release consumes UAT truth; it does not rewrite UAT state or strengthen QA/UAT provenance.

A downstream deployment workflow may rely on `READY_FOR_RELEASE` only from a `CURRENT` exact Release Gate record revision/digest (or equivalent immutable identity). A logical `REL-<id>` or readiness label alone is insufficient. Material controlling-input changes make the prior record stale; preserve it historically and create a new superseding record revision after re-evaluation rather than rewriting the old decision. If canonical record identity cannot be established, do not fabricate it; the release assessment may be preserved but deployment handoff remains unverified/partial.
