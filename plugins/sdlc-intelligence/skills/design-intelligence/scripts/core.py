...
def load_source_manifest():
    """Load bundled provenance/freshness metadata without network access."""
    try:
        with open(SOURCE_MANIFEST_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
    except (OSError, json.JSONDecodeError):
        return {
            "authority": {"class": "ADVISORY_LOCAL_CORPUS", "canonical": False},
            "source_snapshot": {
                "archive": "UNKNOWN",
                "sha256": "UNKNOWN",
            ...
            "freshness": {
                "mode": "UNKNOWN",
                "live_current_claim": False,
                "technical_guidance_status": "REQUIRES_CURRENT_VERIFICATION",
            },
...
def evidence_envelope(technical=False):
    """Return the bounded evidence authority/freshness contract for a result."""
    manifest = load_source_manifest()
    authority = manifest.get("authority", {})
    snapshot = manifest.get("source_snapshot", {})
    freshness = manifest.get("freshness", {})
    envelope = {
        "authority": authority.get("class", "ADVISORY_LOCAL_CORPUS"),
        "canonical": bool(authority.get("canonical", False)),
        "source_snapshot": snapshot.get("archive", "UNKNOWN"),
        "source_sha256": snapshot.get("sha256", "UNKNOWN"),
        "source_git_provenance": snapshot.get("git_provenance", "UNAVAILABLE"),
        "freshness": freshness.get("mode", "UNKNOWN"),
        "live_current_claim": bool(freshness.get("live_current_claim", False)),
        "manifest_status": manifest.get("manifest_status", "UNKNOWN"),
        "decision_authority": "CALLER_RETAINS_AUTHORITY",
    }
    if technical:
        envelope.update({
            "implementation_owner": authority.get(
                "technical_implementation_owner", "frontend-engineering"
            ),
            "technical_guidance": freshness.get(
                "technical_guidance_status", "REQUIRES_CURRENT_VERIFICATION"
            ),
            "verification_sources": freshness.get("technical_verification_sources", []),
        })
    return envelope
...
def search(query, domain=None, max_results=MAX_RESULTS):
    ...
    out = {
        ...
        "count": len(results),
        "results": results,
        "evidence": evidence_envelope(technical=(domain == "react")),
    }
    if auto_detected:
...
def search_stack(query, stack, max_results=MAX_RESULTS):
    ...
    out = {
        ...
        "count": len(results),
        "results": results,
        "evidence": evidence_envelope(technical=True),
    }
    if not results:
        out["suggestions"] = _suggest_terms(bm25, query)
    return out
