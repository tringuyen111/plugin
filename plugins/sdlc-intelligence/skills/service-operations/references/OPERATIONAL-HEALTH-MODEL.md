...
A Deployment Execution Record may hand off:
...
Normal operations consumes that **exact handoff** after the deployment transaction closes. Do not
keep `deploy-release` active merely to poll health. Conversely, do not steal rollback/roll-forward
ownership while the deployment transaction is still active.
...
-> close recheck | continue monitoring | routine action | incident/deploy/engineering handoff
```
