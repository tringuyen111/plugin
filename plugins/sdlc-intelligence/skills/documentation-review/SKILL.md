...
Review one fixed documentation target and return reader-centered findings plus a Documentation review verdict. Review does not silently edit the frozen target. If correction is also authorized, freeze the review result first; subsequent documentation authoring is a separate job and does not require a handoff artifact merely because it occurs in the same session. Read [Review Format](REVIEW-FORMAT.md) for the output contract.
...
Claim independent/attested review only when the project requires and actually provides the corresponding independent execution/governance boundary. Missing independence never prevents an ordinary direct Documentation Review unless independence itself is part of the requested proof burden.
