...
A native Skill with the required **description** and `policy.allow_implicit_invocation: true` in `agents/openai.yaml`. The host may select it autonomously when the description matches, and direct user selection still works. Implicit invocation is a policy choice, not the presence of the description: every native Skill keeps a description. Use it only when autonomous discovery is part of the capability contract and neighbour trigger competition is acceptable.

_Avoid_: ability, tool, capability

### User-Invoked

A native Skill with `policy.allow_implicit_invocation: false` in `agents/openai.yaml`. It still keeps the required **description**, but the host must not auto-select it. A human may select it directly and an authorized router/orchestrator may select it explicitly by canonical identity. The trade-off is reduced trigger competition versus more human/orchestration **cognitive load**; do not claim zero metadata/context cost.

_Avoid_: procedure, workflow, command

### Description

Required machine-readable Skill metadata describing what the Skill does and when it applies. For a **model-invoked** Skill it is host discovery/trigger input; for an explicit **user-invoked** Skill it remains accurate identity for UI, direct selection, and orchestration. Its presence is not the invocation axis; `agents/openai.yaml` policy controls implicit invocation in this native distribution.
...
The token/attention cost of discovery metadata plus whatever Skill body and references the host loads after selection. Exact host loading is runtime-dependent. More **model-invoked** Skills also increase trigger competition; explicit **user-invoked** Skills reduce that competition but still retain required metadata.
...
A Skill whose job is to select one canonical owner among neighbouring Skills and transfer execution under explicit routing gates. It may itself be implicit or explicit. In this native distribution it may select an explicit **user-invoked** Skill by canonical identity; routing does not grant permission to imitate that owner. The cure for **cognitive load** when explicit Skills multiply is one discoverable routing surface, not duplicate ownership.

_Avoid_: dispatcher, menu, registry, index, router procedure

### Granularity

How finely you divide Skills. Finer division spends one of two loads: more **model-invoked** Skills increase discovery metadata and trigger competition; more explicit **user-invoked** Skills increase **cognitive load** and orchestration complexity. Two cuts guide the division. By **invocation**, split off an implicit Skill only where a distinct **leading word** should trigger autonomous selection. By **sequence**, split a run of **steps** where a step's **post-completion steps** need hiding, since isolating it in its own context clears what follows. Beware the reverse: merging sequences exposes each step's post-completion steps to what follows, inviting premature completion.
...
**Reference** that lives outside the Skill system — a plain file, no Skill **description**, no **steps**, not independently invocable — that any Skill can point at. Use it for shared knowledge that should not become a discovery surface or owner. Explicit Skills may both consume the same external reference without invoking each other.
...
A leading word serves **predictability** twice. In the body it anchors **execution** — the agent reaches for the same behaviour every time the concept appears, and inside flat reference it focuses attention on a class of thing to look for, recruiting the right checks each run. In an implicit Skill **description** it can anchor autonomous **invocation**; in an explicit Skill description it anchors recognizable identity for direct/orchestrated selection. Use leading words that match how the capability is actually selected.
