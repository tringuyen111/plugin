...
9. Post-deployment continuation
...
A write with no read-after-write/consumed-state equivalent remains unverified; provider acknowledgement cannot substitute for observed target/consumed state.

## 5. Failure taxonomy

Classify enough to choose a safe next action:

- **PRECONDITION_STALE** — candidate/environment/provider-binding/current state changed;
...
transaction; otherwise require the relevant release/authority preconditions to be re-established before continuation.
...
- `incident-response` continuation when active incident command/stabilization is material; `incident-learning` when stabilized/historical causal learning is the terminal job.

Continue only the concerns that remain material. Procedure/documentation updates may switch to `AUTHOR_PROCEDURE`; normal monitoring may switch to `OPERATE_SERVICE`; active incident command transfers to the incident capability. Do not manufacture lifecycle work merely because deployment completed.
