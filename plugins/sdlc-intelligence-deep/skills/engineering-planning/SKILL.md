...
8. **Derive the current executable frontier without manufacturing a handoff.** A frontier item must have ready prerequisites, explicit protected gaps, observable outcome, runtime/artifact entrypoint, proof boundary, and no stale planning truth. Planning itself does not implement, but when the user's active outcome includes execution, the same capable/authorized session may continue through `implement`, domain engineering, `devops-engineering`, or the host runtime. That capability transition is not a handoff.
...
- **Implementation requested from a ready frontier:** return the bounded planning truth to the active job; the same capable/authorized session may continue through `implement` or the relevant engineering specialist without a handoff artifact. Planning does not rewrite identity or dependencies to make execution convenient.
