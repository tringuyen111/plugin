...
description: "Plan multi-session, decision-fogged, or coordination-sensitive engineering work from verified current-system truth and authorized target truth into the smallest executable and evaluable frontier. Use for technical planning, decision-path discovery, decomposition, dependency/proof topology, durable work-graph reconciliation, or replanning after source/decision/evidence changes. Let small clear work pass directly to execution without planning artifacts. Do not execute implementation or agents, invent protected Product/Requirements/Architecture/QA/release truth, or replace traceability or handoff mechanics."
---
...
|---|---|---|
| `DIRECT` | the objective, source target, constraints, and proof are already clear enough for one bounded execution loop | no map/spec/ticket ceremony; return the bounded execution truth to the actual execution owner |
...
8. **Derive the current executable frontier and stop at the boundary.** A frontier item must have ready prerequisites, explicit protected gaps, observable outcome, runtime/artifact entrypoint, proof boundary, and no stale planning truth. Execution belongs to `implement`, domain engineering, `devops-engineering`, or the host runtime as appropriate.
...
- **Implementation requested from a ready frontier:** `implement` or the relevant engineering specialist executes; Planning does not rewrite identity or dependencies to make execution convenient.
