...
function generateCSS(tokens) {
  const hasLayerRoots = ['primitive', 'semantic', 'component', 'dark']
    .some((key) => Object.prototype.hasOwnProperty.call(tokens, key));
  const primitiveSource = hasLayerRoots ? (tokens.primitive || {}) : tokens;
  const primitivePrefix = hasLayerRoots ? ['primitive'] : [];
  const primitive = flattenTokens(primitiveSource, tokens, primitivePrefix);
