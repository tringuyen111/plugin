...
- When approved project truth requires token/design-system implementation, select the exact decision-local module from **Design-system implementation interfaces** below. Do not browse/load the whole design-system folder, and do not treat bundled examples as Product/Design truth.

## Design-system implementation interfaces

Use these only after Product/Design truth for the affected visual semantics already exists. A module may refine frontend implementation mechanics; it cannot create or approve palette, spacing, typography, radius/shadow, motion, state styling, component anatomy, or other visual-system truth.

| Decision frontier | WHEN | WHY | TARGET | RETURN |
|---|---|---|---|---|
| Token mapping / migration | Approved token roles/values exist, but their code mapping, alias graph, theme projection, or migration seam can change implementation correctness | Preserve the approved token contract while mapping it to the repository's real styling surface | [Token Mapping and Migration](references/design-system/token-mapping.md) | token source/authority + mapping/alias decision + affected consumers/migration seam + proof target; or the exact missing Design/system decision |
| Component states / variants | Approved states/variants exist, but their DOM/CSS/component-state/accessibility implementation can change correctness | Map approved visual/state truth onto browser semantics without inventing state styling | [Component State and Variant Implementation](references/design-system/component-state-implementation.md) | state/variant implementation map + semantic/accessibility behavior + unresolved visual gap + proof target |
| Framework adapter | Repository evidence establishes a styling framework and approved tokens must be projected into it | Adapt approved token semantics to the actual framework without making that framework or its conventions authoritative | [Framework Token Integration](references/design-system/framework-integration.md) | proven framework seam + adapter/config mapping + generated/changed surfaces + proof target |

If the current question does not need one of these returns, do not load that module.

## Entry gate

Establish the exact frontend outcome, inspected production/browser seam, fixed Product/behavior/visual truth that constrains the change, relevant technical decisions, source-write scope, and a falsifiable browser/runtime proof target. A tracker, work item, semantic ledger, or parent Implement wrapper/invocation is not required.
...
If new Product/Design/technical truth, shared-seam evidence, state-lifecycle evidence, or browser/runtime proof invalidates an earlier frontend premise, reopen the earliest affected state/composition/runtime/proof decision and its material dependents. Preserve independent established UI behavior and proof; widen re-entry only when the changed premise is shared/root truth for the bounded frontend unit.
