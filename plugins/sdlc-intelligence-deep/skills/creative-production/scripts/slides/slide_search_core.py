...
# ============ CONFIGURATION ============
DATA_DIR = Path(__file__).resolve().parents[2] / "data" / "slides"
MAX_RESULTS = 3
...
CSV_CONFIG = {
    "strategy": {
        ...
        "output_cols": ["strategy_name", "keywords", "slide_count", "structure", "goal", "audience", "tone", "narrative_arc", "emotion_arc", "sparkline_beats", "sources"]
    },
    "layout": {
        "file": "slide-layouts.csv",
        "search_cols": ["layout_name", "keywords", "use_case", "recommended_for"],
        "output_cols": ["layout_name", "keywords", "use_case", "content_zones", "visual_weight", "cta_placement", "recommended_for", "avoid_for", "card_variant", "metric_style", "quote_style", "grid_columns", "visual_treatment"]
    },
    "copy": {
        "file": "slide-copy.csv",
    ...
    "chart": {
        ...
        "output_cols": ["chart_type", "keywords", "best_for", "data_type", "when_to_use", "when_to_avoid", "max_categories", "slide_context", "accessibility_notes"]
    },
    "layout-logic": {
        "file": "slide-layout-logic.csv",
        "search_cols": ["goal", "emotion", "layout_pattern", "direction", "visual_weight"],
        "output_cols": ["goal", "emotion", "layout_pattern", "direction", "visual_weight", "break_pattern", "use_bg_image"]
    },
    "typography": {
        "file": "slide-typography.csv",
        "search_cols": ["content_type", "primary_size", "secondary_size", "weight_contrast"],
    ...
    "color-logic": {
        ...
        "search_cols": ["emotion", "background", "text_color", "accent_usage", "card_style"],
        "output_cols": ["emotion", "background", "text_color", "accent_usage", "use_full_bleed", "gradient", "card_style"]
    },
    "background": {
        "file": "slide-backgrounds.csv",
        "search_cols": ["slide_type", "image_category", "overlay_style", "text_placement", "search_keywords"],
