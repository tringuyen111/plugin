...
from capture_contract import (
    ...
    validate_resolution_record_binding,
)
...
def main() -> int:
    ...
    try:
        ...
        resolution_binding = job.get("capability_resolution")
        resolution_record = None
        if resolution_binding is not None:
            resolution_record = validate_resolution_record_binding(
                resolution_binding,
                job["executor"],
                root=root,
            )
    except (OSError, ValueError, KeyError, TypeError) as error:
        emit({"ok": False, "stage": "resolution", "error": str(error)})
        return 1
    if args.validate_only:
        payload = {
            ...
            "shot_count": len(job["shots"]),
            "selection_mode": "RESOLUTION_RECORD" if resolution_record is not None else "DIRECT_EXECUTOR",
        }
        if resolution_record is not None:
            payload["resolution_status"] = resolution_record["status"]
            payload["resolution_record_sha256"] = resolution_binding["record_sha256"]
        emit(payload)
        return 0
    ...
    if resolution_binding is not None:
        manifest["capability_resolution"] = dict(resolution_binding)
