...
It **does not own deep corpus synthesis**. When a corpus is large or heterogeneous enough that coding,
theming, triangulation, sample-shape assessment, or synthesis confidence is itself decision-material,
Product Discovery must not imitate that separate method. Consume qualified synthesis from an available
capability/artifact when one exists; otherwise name the unsynthesized evidence question and how it
limits or blocks the current recommendation. Missing a sibling named `research-synthesis` is not itself
...
Recording a cross-owner assumption does not transfer decision authority. A sibling capability may provide evidence or continue a separate owner job, but Product Discovery must still classify the missing truth and finish its own bounded judgment when that truth is not required for the current scope. A missing sibling name alone never causes `BLOCKED`.
...
Product Discovery defines the learning need and evidence semantics. It does not invent an execution capability, provider, or accountable owner when project truth does not supply one.
...
| Technical feasibility asserted by Product | Record the assumption/dependency and request evidence from the accountable technical owner when that truth is needed. |
