...
Choose **not to split** when the proposed children only expose component/team boundaries,
when each child would be unusable without the others, when a dependency changes
implementation order but not the actor-value unit, or when the variation is better
expressed as an acceptance/rule condition inside one coherent capability. A large Story
that still has one indivisible actor outcome should remain one Story until a real value
boundary is found; do not manufacture smaller Stories merely to satisfy a size heuristic.
...
- the **Acceptance Criteria branch** can define observable acceptance without inventing scope;
