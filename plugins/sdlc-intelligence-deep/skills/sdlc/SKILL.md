...
4. **Use the most specific installed capability for domain work.** Native Skill discovery and the Skill descriptions are the capability-selection surface. Prefer the narrow capability that owns the actual job; do not consult or maintain a central Delivery route table. Loading another Skill adds the procedural knowledge needed for that job; it does not imply a simulated organizational handoff.
