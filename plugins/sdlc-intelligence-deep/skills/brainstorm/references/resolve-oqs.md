...
- The current durable brainstorm artifact or the full consolidated conversational brainstorm state must be available.
...
Return or maintain the latest consolidated semantic brainstorm state in conversation. Materialize Markdown only if the user selects it as an output. Do not claim the repo file changed.
